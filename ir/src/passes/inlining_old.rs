...
pub struct InliningOld {
    work_stack: Vec<NodeIndex>,
}

impl VisitContextOld for InliningOld {
    type Graph = MirGraph;
...
impl Pass for InliningOld {
    type Input<'a> = MirOld;
    type Output<'a> = MirOld;
    type Error = CompileError;

    fn run<'a>(&mut self, mut ir: Self::Input<'a>) -> Result<Self::Output<'a>, Self::Error> {
        let mut context = InliningOld::new();
        VisitOld::run(&mut context, &mut ir.constraint_graph_mut());
        Ok(ir)
    }
}

impl VisitDefaultOld for InliningOld {}
