...
use crate::ir2::{
    Add, Boundary, Call, Enf, Fold, For, Graph, If, IsParent, LeafNode, Link, Matrix, MiddleNode, Mul, NodeType, RootNode, Scope, Sub, Vector
};
...
pub fn duplicate_node(node: Link<NodeType>) -> Link<NodeType> {
    match node.borrow().deref() {
        NodeType::RootNode(RootNode::Graph(graph)) => {
            ...
            let new_children = children.iter().map(|child| duplicate_node(*child)).collect();
        ...
        NodeType::MiddleNode(middle_node) => {
            match middle_node {
                ...
                MiddleNode::For(for_node) => {
                    ...
                    let new_iterators = iterators.iter().map(|iterator| duplicate_node(iterator)).collect();
                ...
                MiddleNode::Matrix(matrix) => {
                    ...
                    for row in children.iter() {
                        ...
                        let new_row = row_children.iter().map(|child| duplicate_node(*child)).collect();
