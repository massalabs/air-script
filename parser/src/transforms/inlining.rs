        ...
        for (iterable, binding) in lc.iterables.iter().zip(lc.bindings.iter().copied()) {
            let abstract_value = match iterable {
                ...
                Expr::Range(range) => {
                    ...
                    Expr::Const(Span::new(
                        span,
                        ConstantExpr::Scalar(ScalarType::Uint, (range.start + index) as u64),
                    ))
                },
    ...
    fn populate_evaluator_rewrites(
        ...
        for (arg, segment) in args.iter().zip(params.iter()) {
            match arg {
                ...
                Expr::Const(Span { item: ConstantExpr::Vector(_, items), .. })
                    if items.is_empty() =>
                {
                    continue;
                },
